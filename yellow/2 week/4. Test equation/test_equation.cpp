...
void TestRootCount() {
	...
	// Применяется для указания диапазона действительных чисел.
	uniform_real_distribution<> unif(-10, 10);

	// Как видите, оператор() принимает в качестве параметра ГПСЧ и возвращает СЧ из данного диапазона.
...
void TestOneRoot() {
	...
	for(size_t i = 0; i < 100; i++) {
		...
		// showpos из iomanip подставляет плюсы
